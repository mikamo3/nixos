{ config, lib, pkgs, ... }:
{
  xsession.windowManager.i3 = {
    ...
    config =
      {
        startup = [
          { command = "exec i3-msg workspace 1"; always = true; notification = false; }
        ];
        bars = [
          {
            position = "top";
            ...
            statusCommand = ''
              bumblebee-status \
               -m title date time xrandr bluetooth2 pipewire \
               -p date.format="%Y-%m-%d" time.format="%T"\
               interval=1 \
          ...
          }
          {
            ...
            statusCommand = ''
              bumblebee-status \
               -m uptime hostname traffic cpu memory\
               -p interval=1 \
               -t solarized-powerline \
            '';
          }
